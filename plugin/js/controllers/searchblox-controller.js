...
angular.module('searchblox.controller', [])
    .controller('searchbloxController', ['$rootScope', '$scope', '$http', '$location', 'searchbloxService', 'searchbloxFactory', 'facetFactory', '$q', '$timeout', '$sce','$mdDialog','$mdMedia','$mdToast','$mdConstant','Mailto',
        function ($rootScope, $scope, $http, $location, searchbloxService, searchbloxFactory, facetFactory, $q, $timeout, $sce,$mdDialog,$mdMedia,$mdToast,$mdConstant,Mailto) {// 'autoCompleteFactory',
            ...
            $scope.CartDocObj = [];
            $scope.downloadArchive = null;
            $scope.savedSearches = [];
            $scope.doc = [];
            $scope.docUpdated = 0;
            $scope.cartItems = 0;
            $scope.mdChipsSeparator = [$mdConstant.KEY_CODE.ENTER, $mdConstant.KEY_CODE.COMMA];
            ...
            $scope.doSearch = function () {
                ...
                if($scope.query == "undefined" || !$scope.query.trim()) {
                    $scope.parsedSearchResults.found = -1;
                    $scope.startedSearch = true;
                    return;
                }

                // Show Cart Count From Local Storage
                if(localStorage.cartObjCount) {
                    $scope.cartItems = localStorage.cartObjCount;
                }

                // Show Cart From Local Storage
                if(localStorage.cartObj) {
                    $scope.CartDocObj = angular.fromJson(localStorage.cartObj);
                }

                $scope.setDownloadArchiveUrl(); // Show Download Archive Option

                // Show Saved Searches
                if(localStorage.searchQueries) {
                    $scope.savedSearches = angular.fromJson(localStorage.searchQueries);
                }
                ...
                var urlParams = searchbloxService.getUrlParams(searchUrl, $scope.query,
                    ...
                    $scope.searchCollections = searchResults.data.searchform.collections;
            ...
            // Store Search Query On Local storage

            $scope.storeQueryOnLocalStorage = function() {

                // If Not In local storgae then save query
                if(!localStorage.searchQueries && $scope.query) {
                    var saveSearch = [];
                    saveSearch[0] = $scope.query;
                    localStorage.searchQueries = angular.toJson(saveSearch);
                } else {
                    var searchQueries = angular.fromJson(localStorage.searchQueries);

                    // If Search Query not already in the storage
                    if($scope.query && searchQueries.indexOf($scope.query) === -1) {

                        //If greater than 10 search queries
                        if(searchQueries.length >= 10) {
                            searchQueries.shift();
                        }

                        searchQueries.push($scope.query);
                        localStorage.searchQueries = angular.toJson(searchQueries);
                    }
                }
            ...
            // Search Saved function
            $scope.doSearchSaveQuery = function (query) {
                $scope.query = query;
                $scope.doSearch();
            }


            // Show More Like This Search
            $scope.showMoreLikeThis = function (doc) {
                var colID = doc.col;
                var docId = doc.id;

                if(!colID)
                    return;
                var collectionId = "&mlt_col="+colID;
                var urlParams = searchbloxService.getMoreLikeThisUrlParams(searchUrl, docId,collectionId);
                searchbloxFactory.getResponseData(urlParams).then(function (searchResults) {
                    if(!searchResults.data) {
                        $scope.showDialog('No Results Found!');
                        return;
                    }

                    $scope.moreLikeThisResults = searchbloxService.parseResults(searchResults.data, $scope.facetMap, $scope.dataMap);
                    if($scope.moreLikeThisResults.found > 0) {
                        $scope.showDialog(0,"similar");
                    } else {
                        $scope.showDialog('No Results Found!');
                    }
                });
            }
            $scope.showDialog = function(notFound,template) {
                var alert;
                if(notFound != 0) {
                    ...
                    var dialogTemp = {"similar" : "find-similar.html", "cart-docs" : "cart-docs.html"
                    ,"note-tags": "add-note-tags.html"};

                    $mdDialog.show({
                        templateUrl: 'views/dialog-template/'+dialogTemp[template],
                        escapeToClose : true,
            ...
            // Search Saved function
            $scope.setDownloadArchiveUrl = function () {
               var urlParams = searchbloxService.getDownloadArchiveUrl(searchUrl, $scope.query);
                if(urlParams) {
                    searchbloxFactory.getResponseData(urlParams).then(function (response) {
                        if(response.data.indexOf("<!DOCTYPE") === -1) {
                            $scope.downloadArchive = urlParams; // If Response Is a Zip File name and not the default page
                        } else {
                            $scope.downloadArchive = null;
                        }
                    });
            ...
            // Add Docs To cart
            $scope.addDocToCart = function (docUrl) {
                docUrl = docUrl.url;

                if($scope.CartDocObj) {

                    if(!docUrl || $scope.CartDocObj.indexOf(docUrl) !== -1) {
                        return;
                    }
                } else {
                    $scope.CartDocObj = [];
                ...
                $scope.showSimpleToast('Document Added!');
                $scope.CartDocObj.push(docUrl);
                $scope.cartItems++;

                $scope.updateCartObj(); // Update Local Storage
            };

            // Add Docs To cart
            $scope.showCartDoc = function () {
                var alert;
                var DocLinks = "";

                angular.forEach($scope.CartDocObj, function(value, key) {
                  DocLinks += value + "\n";
                });

                if(!$scope.cartItems || $scope.cartItems == 0) {
                    $scope.showDialog('No Items in cart!');
                } else {
                    var options = {
                        subject: "Cart Documents",
                        body: "Document links: \n\n"+DocLinks
                    };

                    var href = Mailto.url("", options);
                    $scope.mailToDocsLink = href;
                    $scope.showDialog(0,'cart-docs');
                }
            };
            ...
            $scope.clearCartItem = function (item) {
               if(item == "all") {
                    ...
                    var index = $scope.CartDocObj.indexOf(item);
                    $scope.CartDocObj.splice(index, 1);
                    $scope.cartItems--;
               }
            ...
            $scope.updateCartObj = function(val) {

                if($scope.CartDocObj) {
                    localStorage.cartObj = angular.toJson($scope.CartDocObj);
                } else {
                    if(localStorage.cartObj) {
                        delete localStorage.cartObj;
                    }
                }

                if($scope.CartDocObj && $scope.CartDocObj.length) {
                    localStorage.cartObjCount = $scope.CartDocObj.length;
                } else {
                    localStorage.cartObjCount = 0;
                }
            }

             // Add Docs To cart
            $scope.addNotesTagsToDoc = function (result) {
                ...
                $scope.showDialog(0,'note-tags');
            }

            $scope.setNotesTagsForDoc = function(result,showInPopup) {
                
                if(!result["@id"] || !result.col ) {
                    return false;
                }

                var collectionId = "idx00" + parseInt(result.col);
                var docId = encodeURIComponent(result["@id"]);

                $scope.getCollectionType(docId,collectionId).then(function (response) {

                    if(!response) {
                        return false;
                    }

                    var collectionType = response;

                    var requestObj = searchbloxService.getupdateNotesTagsRequest(docId,collectionId,collectionType,result);

                    if(requestObj) {
                        searchbloxFactory.updateRequestData(requestObj).then(function (response) {
                            if(response) {
                                $scope.showSimpleToast("Document updated!",showInPopup)
                            } else {
                                $scope.showSimpleToast("Error updating document!")
                            }
                        });
                    }
                });
            ...
            $scope.showSimpleToast = function(msg,showInPopup) {
                if(showInPopup) {
                     $mdToast.show(
                      $mdToast.simple()
                        .textContent(msg)
                        .position('top')
                        .parent(document.getElementById("toast-container-popup"))
                        .hideDelay(2000)
                    );
            ...
            $scope.getCollectionType = function(docId,collectionId) {

                var urlParams = searchbloxService.getCollectionTypeUrl(docId,collectionId);
                ...
                if(urlParams) {
                    searchbloxFactory.getResponseData(urlParams).then(function (response) {
                        ...
                        deferred.resolve(response.data._type);
                    }).catch(deferred.reject);
                } else {
                    deferred.reject('Url params is not defined');
                }
