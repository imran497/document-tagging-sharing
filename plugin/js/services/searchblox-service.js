...
angular.module('searchblox.service', [])
    .service('searchbloxService', ['$rootScope', function ($rootScope) {
        ...
        // function for generating Find Similar url
        this.getMoreLikeThisUrlParams = function (searchBaseurl, docId,collectionIds) {
            if (typeof(docId) !== "undefined" && docId) {
                var urlParam = searchBaseurl + "?xsl=json"+collectionIds+"&mlt_id="+docId;
                return urlParam;
            }
        }

        // function for generating Download Archive Url
        this.getDownloadArchiveUrl = function (searchBaseurl, query) {
            if (typeof(query) !== "undefined" && query !== null) {
                var urlParam = searchBaseurl + "?downloadAsZipAlone=true&query="+query;
                return urlParam;
            }
        }

        // function for generating Download Archive Url
        this.getCollectionTypeUrl = function (docId, collectionId) {
            if (typeof(docId) !== "undefined" && docId && collectionId) {
                var urlParam = location.protocol + "//" +  location.hostname + ":9200" + "/" + collectionId + "/_all/"+ docId;
                return urlParam;
            }
        }

        // function for updating Docs With Elastic Search API
        this.getupdateNotesTagsRequest = function (docId,collectionId,collectionType,result) {

            var requestObj = {};

            if (typeof(docId) !== "undefined" && docId && collectionId && collectionType) {

                requestObj.urlParam =  location.protocol + "//" +  location.hostname + ":9200" + "/" + collectionId + "/" + collectionType + "/" + docId + "/_update";
